 ...
 * @validator range
 */
export function range<T extends RangeInput>(minValue: T, maxValue: T, options?: ValidationOptions) {
  return validator<T>(
    'range',
  ...
  return validator<RangeInput, RangeInput>(
    ...
    function (input: RangeInput, context: Context, _this): Nullish<RangeInput> {
      if (
        ...
        return input;
      if (minValue instanceof Date && input instanceof Date && input > minValue) return input;
      if (
        typeof minValue === 'string' &&
        typeof input === 'string' &&
  ...
  return validator<RangeInput, RangeInput>(
    ...
    function (input: unknown, context: Context, _this): Nullish<RangeInput> {
      if (
        ...
        return input;
      if (maxValue instanceof Date && input instanceof Date && input < maxValue) return input;
      if (
        typeof maxValue === 'string' &&
        typeof input === 'string' &&
