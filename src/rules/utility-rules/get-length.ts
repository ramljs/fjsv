...
type ExtractLengthInput = string | any[] | ArrayBuffer | { length: number } | { size: number };
...
export function getLength() {
  return validator<number, ExtractLengthInput>(
    ...
    function (input: any, context: Context, _this): Nullish<number> {
      ...
      if (input && typeof input === 'object' && typeof input.length === 'number') return input.length;
      if (input && typeof input === 'object' && typeof input.size === 'number') return input.size;
