...
describe("isTuple", function () {
  ...
  it("should validate value is an array", function () {
    ...
    expect(() => isTuple([isBoolean()])(undefined)).toThrow('Value must be a tuple');
    expect(() => isTuple([isBoolean()])(null)).toThrow('Value must be a tuple');
    expect(() => isTuple([isBoolean()])(5 as any)).toThrow('Value must be a tuple');
    expect(() => isTuple([isBoolean()])(NaN as any)).toThrow('Value must be a tuple');
  });

  it("should validate items according to item rule", function () {
    expect(() => isTuple([isInteger()])(['1'])).toThrow('Value at [0] must be an integer number');
  });
