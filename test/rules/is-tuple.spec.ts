...
describe("isTuple", function () {
  ...
  it("should validate value is an array", function () {
    ...
    expect(() => isTuple([isBoolean()])(undefined)).toThrow('Value is not a valid tuple');
    expect(() => isTuple([isBoolean()])(null)).toThrow('Value is not a valid tuple');
    expect(() => isTuple([isBoolean()])(5 as any)).toThrow('Value is not a valid tuple');
    expect(() => isTuple([isBoolean()])(NaN as any)).toThrow('Value is not a valid tuple');
  });

  it("should validate items according to item rule", function () {
    expect(() => isTuple([isInteger()])(['1'])).toThrow('`Item[0]` is not a valid integer number');
  });
