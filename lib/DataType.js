...
class DataType {
  ...
  compile(options = {}) {
    ...
    /* istanbul ignore else */
    const preparedOptions = this.factory.prepareCompileOptions ?
        this.factory.prepareCompileOptions(options) : options;
    // Lookup for cached validator for same options

    const optsstr = objectHash(options);
    ...
    const fn = this.factory.compile(this, preparedOptions, options);
  ...
  set(attribute, value) {
    this.factory.set(this, attribute, value);
  }
