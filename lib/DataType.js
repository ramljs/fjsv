...
class DataType {
  ...
  _flatten(target, name) {
    ...
    if (Array.isArray(target))
      target.forEach(t => t.solidify());
    else target.solidify();
    return target;
  }

  _generateValidateFunction(options) {
    let fn;
    if (this.__generatingValidateFunction) {
      return (...args) => fn(...args);
    }
    this.__generatingValidateFunction = true;
    try {
  ...
  return value;
      ...
      delete this.__generatingValidateFunction;
    }
  }
