...
class ObjectType extends AnyType {
  ...
  _generateValidationCode(dataType, options) {
    ...
    if (propertyKeys.length) {
      ...
      for (const k of propertyKeys) {
        ...
        if (m) {
          rxproperties.push({
            property,
            fn: property.dataType.compile(propOptions),
    ...
    if (typeof additionalProperties !== 'boolean')
      additionalProperties =
          dataType.library.get(additionalProperties)
              .compile({...propOptions, strictFormat: true});
