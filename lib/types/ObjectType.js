  ...
  _generateValidationCode(options, context) {
    ...
    if (propertyKeys.length) {
      ...
      for (const k of propertyKeys) {
        ...
        const m = k.match(/^\/(.*)\/$/);
        const ignoreRequire = (m && property.required == null) ||
...
`;
    ...
    if (unionMatch) {
      ...
      for (let i = 0; i < ${propertyKeys.length}; i++) {
        ...
        if (p.regexp) {       
          const xctx = {...ctx, name: k};
          const jlen = valueKeys.length;
          let h=0;
          for (let j = 0; j<jlen; j ++) {
            if (valueKeys[j].match(p.regexp) && 
    ...
    for (let i = 0; i < keysLen; i++) {
      ...
      if (p) {          
          delete prpVlds[k];
          const vv = p.fn(value[k], (path ? path + ' > ' + k : k), 
              subError, {...ctx, name: k});
          if (vv === Failed) {
      ...
      if (additionalProperties) {
        ...
        data.code += ` else {
            error({
                    message: 'No additional property (' + k + ') accepted'+
                        (path ? ' for '+ path : '') + '.',
      ...
      for (let i = 0; i < keysLen; i++) {
        ...
        if (p.regexp)
          continue;          
        const n = numErrors;
        const vv = p.fn(value[k], (path ? path + ' > ' + k : k), subError, {...ctx, name: k});
        if (numErrors > n)
          ${maxErrors >
