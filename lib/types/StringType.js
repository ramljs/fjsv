...
class StringType extends DataType {
  ...
  _generateValidationCode(options, fnCache) {
    ...
    data.code += `)
      ...
      return error({
          message: 'Value'+(path ? ' for '+ path: '')+' must be a string',
    ...
    if (!enums.has(v))
        return error({
            message: 'Value'+(path ? ' for '+ path: '')+' must be a one of enumerated value',
    ...
    if (v.length < ${this.minLength})
        return error({
            message: 'Minimum accepted length'+(path ? ' for '+ path: '')+
    ...
    if (v.length > ${this.maxLength})
        return error({
            message: 'Maximum accepted length'+(path ? ' for '+ path: '')+
