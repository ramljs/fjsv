...
class ArrayType extends AnyType {
  ...
  _generateValidationCode(dataType, options, orgOptions) {
    ...
    const minItems = dataType.get('minItems');
    const maxItems = dataType.get('maxItems');
    const uniqueItems = dataType.get('uniqueItems');
    const items = dataType.get('items');
    const strictFormat = coalesce(options.strictFormat, dataType.get('strictFormat'));
    const maxErrors = data.variables.maxErrors = options.maxArrayErrors;
    const itemsValidator = data.variables.itemsValidator = items &&
        items.compile(orgOptions);
    if (strictFormat)
      data.code += `
