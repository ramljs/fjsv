...
class TypeLibrary {
    ...
    try {
      ...
      if (typeof schema === 'string') {
        ...
        if (this._internals[name]) {
          ...
          return this._cache[name] = this._internals[name].create(null, {name});
        } else {
  ...
  _createType(instance, schema) {
    ...
    if (schema instanceof DataType) {
      throw new Error('not implemented');
      Object.setPrototypeOf(instance, schema);
      instance.type = [schema];
      instance.attributes = {};
      if (instance.typeName === this._objectTypeName)
        instance.properties = {};
      return instance;
    }
    ...
    if (Array.isArray(schema.type)) {
      ...
      schema.type.forEach((sch, idx) => {
        ...
        if (sch.type === this._unionTypeName) {
          if (!(sch.anyOf && sch.anyOf.length))
            return;
          const anyOf = sch.anyOf.map(t =>
              t instanceof DataType ? t : this.get(t));
          if (!isUnion)
            trgTypes = trgTypes.map(t => t.clone());
          isUnion = true;
          const a = [trgTypes];
          const l = anyOf.length;
          for (let i = 1; i < l; i++)
        ...
        trgTypes.push(this.get(sch));
        stack.pop();
      });

      if (isUnion) {
        this._createType(instance, this.get(this._unionTypeName));
        instance.anyOf = trgTypes;
        instance.mixin(schema);
        return instance;
      } else {
        const base = trgTypes[0];
        base.create(instance, schema);
    ...
    const t = base.create(instance, schema);
    t.merge(base);
    t.merge(schema);
    return t;
  }
